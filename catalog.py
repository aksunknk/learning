...
LessonCatalog = Literal["engineering", "foundation"]
...
def load_lessons() -> None:
    global _foundation, _engineering
...
def lessons_for_catalog(catalog: LessonCatalog) -> list[dict]:
    return _engineering if catalog == "engineering" else _foundation


def find_lesson_any(lesson_id: str) -> dict | None:
    for pool in (_engineering, _foundation):
        for item in pool:
